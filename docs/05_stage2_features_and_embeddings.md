...
flowchart TD
    ...
    CP --> S2c["Stage 2c: 35 Deterministic Lexical, Phonetic, & Structural Features"]
...
### 2.4 Stage 2c: 35 Deterministic Lexical & Structural Features
...
|---|---|---|---|
| **Name Lexical (6)** | `name_exact` | Binary indicator: `norm_name_1 == norm_name_2` | $\{0.0, 1.0\}$ |
...
| **Address Lexical (6)** | `address_exact` | Binary indicator: `norm_addr_1 == norm_addr_2` | $\{0.0, 1.0\}$ |
...
| **Numeric & Postal (4)** | `postal_equal` | Exact postal code match (ZIP/PIN/Code Postal) | $\{0.0, 1.0\}$ |
...
| **Missingness & Flags (4)**| `left_country_missing` | Indicator if S1 country was missing | $\{0.0, 1.0\}$ |
...
| **Contradictions (2)** | `same_name_different_address` | Exact name match but different addresses | $\{0.0, 1.0\}$ |
| | `same_address_different_name` | Exact address match but different names | $\{0.0, 1.0\}$ |
| **Country & Source (3)** | `country_equal` | Canonical country match flag (0=mismatch, 1=match, -1=missing) | $\{-1.0, 0.0, 1.0\}$ |
...
| **Blocker Graph / Context (10)** | `candidate_rank` | Retrieval rank of candidate from Stage 1 (999.0 if missing) | $[1.0, 999.0]$ |
...
#### Merged Auxiliary Embedding Features (Stage 2a & 2b)
When dense or generative models are enabled, auxiliary score columns and corresponding missing indicators are merged into the feature matrix:
- `bge_cosine` and `bge_cosine_missing`: Fine-tuned BGE-M3 rsLoRA cosine similarity and missing indicator.
- `qwen_cosine` and `qwen_cosine_missing`: Optional auxiliary Qwen3-Embedding-0.6B cosine similarity and missing indicator.
- `qwen_matcher_prob` and `qwen_matcher_prob_missing`: Optional Stage 2b generative matcher sliced verdict probability and missing indicator.
- `tfidf_cosine`: Dynamic in-batch TF-IDF cosine similarity fitted per cross-validation fold.
...
```powershell
# Extracts Stage 2c deterministic features + BGE-M3 + optional Qwen embedding features
...
python -m src.pair_features `
    ...
    --candidate-file output/phase1_blocking_train/candidate_pairs.tsv `
    --output-file output/phase2_features_train/pair_features.tsv `
    --provenance-file output/phase1_blocking_train/candidate_provenance.tsv
```
...
1. **Ablation 1 (Lexical Baseline):** Stage 2c deterministic features only (35 features).
