...
|---|---|---|
| ~~Stage 2a encoder~~ | ~~BGE-M3's dense head vs. Qwen3-Embedding-0.6B~~ | **CLOSED — see [`system_architecture.md`](../system_architecture.md) §3.** Committed: fine-tuned BGE-M3 dense head (Stage 2a-i), gated by the two-direction held-out-country check; Qwen3-Embedding-0.6B kept as an optional inference-only auxiliary feature (**Stage 2a-ii**), not a competing primary. Do not re-run this as an open grid. |
...
| Stage 2b Qwen3-0.6B Generative Matcher class imbalance & tuning (stretch-only, see [`04b_qwen3_generative_matcher_spec.md`](04b_qwen3_generative_matcher_spec.md)) | Per-example loss weighting on verdict token vs. class-balanced batch sampling; LoRA rank 64 starting point (`use_rslora=True`, all-linear) | Held-out-country gate (train US, eval India, confirmed France proxy); feature dropped if gate fails |
