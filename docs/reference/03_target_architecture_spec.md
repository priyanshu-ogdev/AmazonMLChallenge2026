...
## Stage 2 — Feature engineering (independent sub-stages, no interdependency)

**2a — Bi-encoder cosine similarity.** Dense encoders score each candidate pair:
- **2a-i (Primary fine-tuned representation):** BGE-M3 LoRA fine-tuned (rank 64, `use_rslora=True`, all-linear, CachedMNRL + 0.10 self-distillation), gated by the two-direction held-out-country retrieval check before being trusted over the off-the-shelf fallback — see [`04_bge_m3_training_spec.md`](04_bge_m3_training_spec.md) and [`stage2_features_and_embeddings.md`](../stage2_features_and_embeddings.md).
- **2a-ii (Auxiliary dense representation):** Qwen3-Embedding-0.6B is an **inference-only auxiliary feature** included only if an ablation proves complementary value; it is subordinate to 2a-i and is not the automatic fallback for a failed BGE adapter (that fallback is unchanged BGE-M3 base).

**2b — Cross-record generative matcher (Qwen3-0.6B causal LM) probability.** A LoRA fine-tuned causal language model (Qwen3-0.6B) scores candidate pairs jointly via sliced verdict-token classification (`Yes`/`No`), replacing legacy Ditto. See [`04b_qwen3_generative_matcher_spec.md`](04b_qwen3_generative_matcher_spec.md). **Cut from v1** — stretch goal only, executed strictly behind steps 1–5 (see [`01_v1_baseline_plan.md`](01_v1_baseline_plan.md)).
