...
- Singleton entities (true zero-match S1 records) score 1.0 if left unmatched and 0.0 if given any
  ...
  top-1 choice (reasoning: `06_optimization_design.md` §2).

## 3. Functional requirements

| ID | Requirement | Rationale | Source layer |
|---|---|---|---|
| FR-1 | Normalize name/address fields identically across S1, S2, S3 before any comparison | Vendor formatting differs; unnormalized comparison undercounts true matches | `01_blocking_stage.md` §1 |
| FR-2 | Generate candidate pairs via multiple independent block keys, unioned | Any single key has a distinct blind spot; union drives recall up, reasoned in `06_optimization_design.md` §1 | `01_blocking_stage.md` §2 |
...
| FR-4 | Score every candidate pair independently with a multi-feature classifier, not a single similarity threshold | Single-feature threshold can't structurally separate "high name / low address similarity" from true matches — see FR capacity argument in `06_optimization_design.md` §2 | `02_matching_stage.md` |
| FR-5 | Support an explicit no-match / abstain outcome per S1 entity | Singleton scoring is binary; forced top-1 choice guarantees wrong answers on every true singleton | `02_matching_stage.md`, PRD §2 |
...
| NFR-4 | Any embedding model used for full-table blocking should be sized for single-pass throughput over 100K+ candidate pairs; reserve larger models (4B/8B class) for shortlist-only reranking | Compute-budget reasoning in `06_optimization_design.md` §4 |
...
- Speculative hyperparameter values presented as final — see `05_hyperparameter_verification_status.md` for why these are deliberately left open pending real data
...
|---|---|---|
| Blocking recall ceiling too low on locale-specific addresses (e.g. France, if confirmed as a real held-out locale) | Address format conventions differ by country (primary brief flags this generically; France-specificity is unconfirmed — see `04_citations.md`) | Locale-specific normalization rules (FR-10), measured recall audit before matching-stage work begins (`05_hyperparameter_verification_status.md` §"blocking recall audit") |
| Overfitting matching threshold to F1 intuition instead of F_0.5 | Default classifier tooling/metrics report F1 by default, not F_0.5 | Explicit threshold sweep reporting F_0.5, singleton-sliced, before finalizing (`02_matching_stage.md`) |
| Shipping code with an incompatible license | LinkTransformer confirmed GPL-3.0 (`04_citations.md`) | Reimplement the technique independently, don't vendor (NFR-1) |
| Over-engineering (fine-tuning, reranking) without measured benefit | Literature precedent (Sodhana paper, etc.) is suggestive, not conclusive for this specific dataset | Incremental build-and-measure: each optional component must show a measured F_0.5 delta on a held-out fold before being kept (`05_hyperparameter_verification_status.md` §3) |

## 8. Open questions requiring real data (not resolvable at PRD stage)
See `05_hyperparameter_verification_status.md` for the full list — blocking thresholds, classifier
